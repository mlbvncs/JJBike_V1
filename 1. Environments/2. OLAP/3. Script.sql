...
--ONLY THE MONTH OF JANUARY IS LOADED IN FACT SALES
...
--A plan change is being made for clients with IDs 1 to 5.
...
--Check the modified clients to verify each client's history. Date fields have been updated.
SELECT * FROM Dimensional.dim_customer WHERE id_customer BETWEEN 1 AND 5;

--VERIFIED THAT THE CLIENTS IN THE FACTUAL LOAD FOR JANUARY ARE POINTING TO THE OLD SKs OF CLIENTS 1, 2, 3, AND 5
SELECT * FROM Dimensional.fact_sales f
    INNER JOIN Dimensional.dim_customer c
        ON f.customer_key = c.customer_key
WHERE c.id_customer BETWEEN 1 AND 5;

--ONLY FEBRUARY IS LOADED IN FATO VENDAS
INSERT INTO Dimensional.fact_sales(seller_key, customer_key, product_key, time_key, quantity, unit_price, total_price, discount)
SELECT
    vdd.seller_key,
    c.customer_key,
...
--NOTICE THAT THE FACT LOAD CUSTOMERS FOR JANUARY ARE POINTING TO THE OLD SKs OF CUSTOMERS 1 TO 5. IN FEBRUARY ONLY CUSTOMER 5 MADE PURCHASES.
--THE FACT RECORDS ALREADY POINT TO THE CUSTOMER'S LAST SK.
SELECT * FROM Dimensional.fact_sales f
    INNER JOIN Dimensional.dim_customer c
        ON f.customer_key = c.customer_key
WHERE c.id_customer BETWEEN 1 AND 5;

--ONLY THE MONTH OF MARCH IS LOADED IN FACT VENDAS
INSERT INTO Dimensional.fact_sales(seller_key, customer_key, product_key, time_key, quantity, unit_price, total_price, discount)
SELECT
    vdd.seller_key,
    c.customer_key,
...
--MAKE A CHANGE TO CLIENT PLAN 3
UPDATE Relational.customers SET customer_status = 'Platinum' WHERE id_customer = 3;

--LOAD CUSTOMER DATA. WILL ONLY PROCESS HISTORY ON MODIFIED RECORDS (CLIENT 3)
WITH S AS (
     SELECT * FROM Relational.customers
),
UPD AS (
...
--CHECK ALL PURCHASES FROM THE CUSTOMERS WE ARE MAPPING (1 TO 5). WE VERIFIED THAT THERE WERE PURCHASES WITH THE OLD SK STATUS AND NOW THERE ARE PURCHASES MADE WITH THE LATEST STATUS (NEW SK STATUS).
SELECT * FROM Dimensional.fact_sales f
    INNER JOIN Dimensional.dim_customer c
        ON f.customer_key = c.customer_key
    INNER JOIN Dimensional.dim_time t

...
DEL AS (
     ...
     WHERE  T.validity_end_date IS NULL
            AND T.id_customer NOT IN (SELECT id_customer FROM S)
     RETURNING T.id_customer
)
...
DEL AS (
     ...
     SET    validity_end_date = current_date
     WHERE  T.validity_end_date IS NULL
            AND T.id_product NOT IN (SELECT id_product FROM S)
     RETURNING T.id_product
)
...
DEL AS (
     ...
     SET    validity_end_date = current_date
     WHERE  T.validity_end_date IS NULL
            AND T.id_seller NOT IN (SELECT id_seller FROM S)
     RETURNING T.id_seller
)
...
-- TEMPORAL JOIN: Matches the correct historic seller version at the time of the sale
INNER JOIN Dimensional.dim_seller vdd
    ON v.id_seller = vdd.id_seller 
   AND v.sale_date >= vdd.validity_start_date 
   AND (v.sale_date <= vdd.validity_end_date OR vdd.validity_end_date IS NULL)
-- TEMPORAL JOIN: Captures the exact historical context of the customer when they purchased
INNER JOIN Dimensional.dim_customer c
    ON v.id_customer = c.id_customer 
   AND v.sale_date >= c.validity_start_date 
   AND (v.sale_date <= c.validity_end_date OR c.validity_end_date IS NULL)
-- TEMPORAL JOIN: Safely tracks the product version even if it was modified or deleted later
INNER JOIN Dimensional.dim_product p
    ON iv.id_product = p.id_product 
   AND v.sale_date >= p.validity_start_date 
   AND (v.sale_date <= p.validity_end_date OR p.validity_end_date IS NULL)
